    ...
    def score_true_labels_and_save(epoch, test, logger, model_name, num_classes):
        ...
        with torch.no_grad():
            for t_idx, (shift_x, shift_y, flip) in enumerate(transformations):
                preds = torch.zeros(dataset_len).to(torch.bool)
                total = 0
                for inputs, targets in tqdm(dataloader):
                    ...
                    preds[idxs] = pred.detach().clone().cpu()

                print(preds)

                pred_file_name = (
                    f"pred_{model_name}_tid{t_idx}.pt"
                    if not test
                    else f"pred_{model_name}_tid{t_idx}_test.pt"
                )

                logger.info(f"Saving {pred_file_name}")
                blob_container = "learning-dynamics-scores"
                container_dir = args.dataset.lower()
                upload_numpy_as_blob(blob_container, container_dir, pred_file_name, preds.numpy(), True)
        return
