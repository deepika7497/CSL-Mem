...
def setup_logger(config, model_name, global_rank):
    ...
    handler = logging.FileHandler(os.path.join(config['log_dir'], f'train_mislabeled_{model_name}_node{global_rank}.log'))
...
def save_model(net, args, config, model_name, epoch):
    ...
    save_to_cloud_loc(state_dict, 'learning-dynamics-models', f"{args.dataset.lower()}/{model_name}_noisy_idx_{args.random_seed}_epoch_{epoch}_noise_{args.label_noise}.ckpt")
