...
from minio_obj_storage import save_to_cloud, get_numpy_from_cloud, upload_numpy_as_blob
...
def setup_logger(config, model_name, global_rank):
    ...
    handler = logging.FileHandler(os.path.join(config['log_dir'], f'{model_name}_node{global_rank}.log'))
...
def save_state(net, optimizer, epoch, best_val_accuracy, best_val_loss, model_name, args, config, global_rank):
    state = {
        'epoch': epoch + 1,
    ...
    if global_rank == 0:
        ...
        save_model(net, args, config, model_name, epoch)

def save_model(net, args, config, model_name, epoch):
    state_dict = net.module.state_dict() if args.parallel else net.state_dict()
    save_to_cloud(state_dict, 'learning-dynamics-models', f"{args.dataset.lower()}/{model_name}_seed_{args.random_seed}_epoch_{epoch}.ckpt")
...
def train(epoch_callback=None):
    ...
    parser.add_argument('--reproducibility', default=True, action='store_true', help='Enable reproducibility settings')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (data, labels) in enumerate(dataset.train_loader):
            ...
            train_total += total
            curr_acc = 100. * train_total / dataset.train_length
            log_training_progress(logger, epoch, batch_idx, train_total, dataset.train_length, curr_acc, losses.avg)
        ...
        scheduler.step()
        save_state(net, optimizer, epoch, best_val_accuracy, best_val_loss, model_name, args, config, global_rank)
        if epoch_callback:
